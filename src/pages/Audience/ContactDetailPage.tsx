...
export default function ContactDetailPage() {
  ...
  // Update breadcrumb when contact loads
  React.useEffect(() => {
    if (contact) {
      const name = contact.name || `${contact.firstName || ''} ${contact.lastName || ''}`.trim() || 'Contact'
      setBreadcrumbOverride(contactId, name)
    }
  }, [contact, contactId, setBreadcrumbOverride])
    ...
    return (
      <PageWrapper isLoading={true}>
        <PageHeaderProfile
          ...
          onBack={() => navigate("/audience/contacts")}
          actions={
  ...
  const handleEdit = () => {
    navigate(`/audience/contacts/${contactId}/edit`)
  }
